  ...
  return (
    <section className="card reveal p-5" aria-labelledby="hourly-forecast-title" style={{ animationDelay: "120ms" }}>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
        ...
        <div className="hourly-day-selector seg scroll-slim max-w-full overflow-x-auto" role="group" aria-label="Forecast day">
          {days.map((day, idx) => (
            <button key={day.id} type="button" role="radio" aria-checked={day.date === selectedDate}
              ...
              className="seg-btn shrink-0">{getDayLabel(day.date, idx)}</button>
          ))}
        </div>
      </div>
      {hours.length === 0 ? (
